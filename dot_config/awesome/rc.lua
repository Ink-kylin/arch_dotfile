...
local editor_code = "/usr/bin/code --no-sandbox --unity-launch"
local editor_markdown = "/usr/bin/obsidian"
local audio_music = "/usr/bin/electron-netease-cloud-music"

-- IDE
local idea_ce = "/usr/bin/idea %f"
local android_studio = "android-studio %f"
local rider = "/opt/developer-kits/ide/rider/bin/rider.sh"
local webstorm = "/opt/developer-kits/ide/webstorm/bin/webstorm.sh"
local clion = "/opt/developer-kits/ide/clion/bin/clion.sh"
local datagrip = "/opt/developer-kits/ide/datagrip/bin/datagrip.sh"
local pycharm = "/opt/developer-kits/ide/pycharm-PC/bin/pycharm.sh"

-- Autostart applications
awful.spawn.with_shell("$HOME/Scripts/autostart/awesome.sh")
awful.spawn.with_shell("picom -b --config  $HOME/.config/picom/picom.conf")

awful.util.terminal = terminal
awful.util.tagnames = { "1", "2", "3", "4", "5" }
awful.layout.layouts = {
    awful.layout.suit.fair, awful.layout.suit.floating, awful.layout.suit.tile,
...
systemmenu = {
    { "Log out", function() awesome.quit() end }, { "Sleep", "systemctl suspend" },
...
awful.util.mymainmenu = freedesktop.menu.build {
    ...
    after = {
        { "Open terminal", terminal }, { "System", systemmenu }
        -- other triads can be put here
    }
}
...
globalkeys = mytable.join( -- personal keys
    awful.key({ modkey, "Shift" }, "Return",
        function() awful.util.spawn(filemanager) end),

    awful.key({ ctrlkey, "Shift" }, "Escape",
        function() awful.util.spawn("xfce4-taskmanager") end),
    ...
    awful.key({ altkey }, "e",
        function() awful.util.spawn(editor_code) end, {
            description = "Visual Studio Code",
            group = "Personal"
        }),

    awful.key({ ctrlkey }, "k",
        function() awful.util.spawn(simple_terminal) end, {
            description = "Kitty",
            group = "Personal"
        }),

    awful.key({ altkey, "Shift" }, "i", function() awful.util.spawn(idea_ce) end,
        { description = "Capable and Ergonomic IDE for JVM", group = "Development" }),
    awful.key({ altkey, "Shift" }, "a", function() awful.util.spawn(android_studio) end,
        { description = "The Drive to Develop", group = "Development" }),
    awful.key({ altkey, "Shift" }, "p", function() awful.util.spawn(pycharm) end,
        { description = "Python IDE for Professional Developers", group = "Development" }),
    -- awful.key({ ctrlkey }, "c", function() awful.util.spawn(clion) end,
    --     { description = "A cross-platform IDE for C and C++", group = "Development" }),
    awful.key({ altkey, "Shift" }, "d", function() awful.util.spawn(datagrip) end,
        { description = "IDE for Databases and SQL", group = "Development" }),
    awful.key({ altkey, "Shift" }, "r", function() awful.util.spawn(rider) end,
        { description = "A cross-platform IDE for .NET", group = "Development" }),
    awful.key({ altkey, "Shift" }, "w", function() awful.util.spawn(webstorm) end,
        { description = "The smartest JavaScript IDE", group = "Development" }),
    awful.key({ altkey, "Shift" }, "o",
        function() awful.util.spawn(editor_markdown) end,
        { description = "Obsidian", group = "Personal" }),
    awful.key({ altkey }, "v",
        function() awful.util.spawn("zy-player.sh") end,
        { description = "ZyPlayer", group = "Personal" }),
    awful.key({ altkey, "Shift" }, "m", function()
        awful.util.spawn("electron-netease-cloud-music")
    end, { description = "网易云音乐非官方客户端", group = "Personal" }),
    awful.key({ altkey }, "F3",
    ...
    -- User programs
    awful.key({ altkey }, "g", function() awful.spawn(browser) end,
    ...
    end, { description = "rofi theme selector", group = "super" }), -- Prompt
    awful.key({ modkey }, "r",
...
awful.rules.rules = {
    ...
    }, -- Floating clients.
    {
        rule_any = {
            ...
            class = {
                ...
                "Wpa_gui", "veromix", "xtightvncviewer", "Xfce4-terminal"
            },
    ...
    {
        rule = { instance = 'libreoffice', class = 'libreoffice-calc' },
        properties = {
            fullscreen = false,
            floating = true,
        }
